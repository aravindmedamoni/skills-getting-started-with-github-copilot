          ...
          <div class="participants">
            ...
            ${details.participants.length > 0
              ? `<ul>${details.participants
                  .map(
                    (email) =>
                      `<li><span>${email}</span><button type="button" class="remove-participant" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(email)}" aria-label="Remove ${email}" title="Remove participant">&#128465;</button></li>`
                  )
                  .join("")}</ul>`
